...
SELECT *
FROM olist_geolocation_dataset; -- Step 2: Confirm the created table

COPY olist_geolocation_dataset  -- Step 3: Copy the CSV file into the created table
...
SELECT *
FROM olist_geolocation_dataset; -- Step 4: Confirm the extracted data
...
-- 	Overall Revenue
-- 		Total Revenue
SELECT TO_CHAR(SUM(price), '$FM999,999,999.00') AS "Total Revenue"
FROM olist_order_items_dataset; -- Total Revenue - $13.6M
...
-- 	Revenue Lost to Canceled Orders

SELECT TO_CHAR(SUM(price), '$FM999,999,999.00') AS "To Be Refunded"
FROM olist_order_items_dataset AS p
JOIN olist_orders_dataset AS o
ON p.order_id = o.order_id
WHERE order_status = 'canceled'; -- Revenue lost to cancelation is $95k


-- 	Total Transaction Value Lost to Canceled Orders
...
-- INSIGHTS
-- Total Revenue is 13.6M
-- Average Order Value is $154.10
-- Lost revenue to Canceled orders is $95k
-- Total transaction value lost to canceled orders is 143k
--------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT 
    ...
    TO_CHAR(SUM(oi.price), 'FM$999,999,999.00') AS "Total Monthly Revenue"
FROM olist_orders_dataset AS o
JOIN olist_order_items_dataset AS oi
ON o.order_id = oi.order_id
GROUP BY "Year", "Month No", "Month Name"
ORDER BY "Year"; 
-- This query returns Monthly Revenue, but it also uncovered 
-- that Nov 2016 has no recorded sales in the dataset
-- mostly due to the absense of sales, 2016 was by far the worst year 
-- for the business in revenue.
...
SELECT 
    ...
    TO_CHAR(COALESCE(SUM(oi.price), 0), 'FM$999,999,999.00')  AS "Total Monthly Revenue",
	ROUND(100.0 * COALESCE(SUM(oi.price), 0) / SUM(COALESCE(SUM(oi.price), 0)) OVER(), 4) "% Revenue"
FROM calendar AS c
...
ORDER BY "% Revenue" DESC; -- Continuing with the analysis, the missing values for Nov, 2016 has been changed to zero to ensure the month is 
						  -- included. Another observation is the next month also has incredibly low sales at $10.90.
						  -- Nov 2017 saw the highest revenue with Sales at 1.010M(7.43%) ot total sales, it is followed closely by April and
							-- May 2018 with sales at 996K(7.3328%) and 996(7.3318%).



-- 		Yearly Revenue
SELECT DATE_PART('year', o.order_purchase_timestamp) AS "Year",
	   TO_CHAR(SUM(oi.price), '$FM999,999,999.00') AS "Revenue",
	   ROUND(100.0 * SUM(oi.price) / SUM(SUM(oi.price)) OVER(), 2) AS "% Revenue"
FROM olist_orders_dataset AS o
JOIN olist_order_items_dataset AS oi
ON o.order_id = oi.order_id
GROUP BY "Year"
ORDER BY "% Revenue" DESC; -- 2018 is the best year even with incomplete records (The record for 2018 ends in Oct) with total revenue at 
							-- $7.4M(54.34%) of total revenue

	   
-- INSIGHTS
-- 2018 saw the highest Sales revenue at 54.34%($7.4M) of total revenue
-- Nov 2017 saw the highest revenue with Sales at 1.010M(7.43%) ot total sales, it is followed closely by April and
-- May 2018 with sales at 996K(7.3328%) and 996(7.3318%).
...
-- CTE 1: One payment type per order (for revenue attribution)
...
-- CTE 2: Total product price per order
order_revenue AS (
    SELECT 
        order_id, 
    ...
    FROM olist_order_items_dataset
    GROUP BY order_id),
-- CTE 3: Total distinct orders that used each payment method
-- This helps avoid undercounting due to the JOIN below
orders_per_payment AS (
    SELECT 
        payment_type,
        COUNT(DISTINCT order_id) AS total_orders
    FROM olist_order_payments_dataset
    WHERE payment_type IS NOT NULL AND payment_type <> 'not_defined'
    GROUP BY payment_type)
-- Final query
SELECT 
    REPLACE (fp.payment_type, '_', ' ') AS "Payment Type",
    opp.total_orders AS "Total Orders (Used)",  -- Power BI-style count
    TO_CHAR(SUM(orv.total_price), '$FM999,999,999.00') AS "Revenue",
    ROUND(100.0 * opp.total_orders / SUM(opp.total_orders) OVER (), 2) AS "Usage Share (%)",
    ROUND(100.0 * SUM(orv.total_price) / SUM(SUM(orv.total_price)) OVER (), 2) AS "Revenue Share (%)"
FROM first_payment fp
JOIN order_revenue orv ON fp.order_id = orv.order_id
JOIN orders_per_payment opp ON fp.payment_type = opp.payment_type
GROUP BY fp.payment_type, opp.total_orders
ORDER BY "Revenue Share (%)" DESC; -- Most customers paid using credit card with total transactions at a staggering 73.92%(76,795) and 
							  -- Revenue at 80%(10.8M)


-- Payment Type Revenue Lost to Canceled Orders ORDERED by Highest Cancelation Rate

SELECT 
    REPLACE(a.payment_type, '_', ' ') AS "Payment Type",
    TO_CHAR(COUNT(DISTINCT a.order_id), 'FM999,999,999') AS "Total Orders",
    SUM(CASE WHEN b.order_status = 'canceled' THEN 1 ELSE 0 END) AS "Canceled Orders", 
    TO_CHAR(SUM(CASE WHEN b.order_status = 'canceled' THEN payment_value ELSE 0 END), '$FM999,999,999') AS "Revenue",
...
-- Freight Value
SELECT TO_CHAR(SUM(freight_value), '$FM999,999,999.00') AS "Total Freight Value"
FROM olist_order_items_dataset;

								 -- INSIGHTS
								 -- Freight value is $2.252

-- State's contribution to Freight Value
SELECT c.customer_state, TO_CHAR(SUM(oi.freight_value), '$FM999,999,999.00') AS "Freight Value", 
	   ROUND(100 * SUM(oi.freight_value) / SUM(SUM(oi.freight_value)) OVER(), 2) AS "% Contribution"
...
ORDER BY SUM(oi.freight_value) DESC; -- SP again accounts for most freight value at 31.92%($719k) of total freight value 
									  -- of $2.252M.
...
-- Step 3: Link the 2 tables to each other. This third table creates a link between the 1st and 2nd table
OrdersBridgeTable AS (
	    SELECT order_id,
			   customer_id
		FROM olist_orders_dataset
)
-- Step 4: Retrieve Customer State, Total Customers, Total Revenue etc.
SELECT cs.customer_state, 
	   COUNT(DISTINCT cs.customer_id) AS "Total Customers", 
	   TO_CHAR(SUM(tr."Total Revenue"), '$FM999,999,999.00') AS "Total Revenue",
	   ROUND(100 * SUM(tr."Total Revenue")/SUM(SUM(tr."Total Revenue")) OVER(), 2) AS "% Revenue",
	   ROUND(100 * COUNT(DISTINCT cs.customer_id)/SUM(COUNT(DISTINCT cs.customer_id)) OVER(), 2) AS "% Customer"
FROM customer_state AS cs
JOIN OrdersBridgeTable AS obt
	ON cs.customer_id = obt.customer_id
JOIN Total_Revenue AS tr
	ON obt.order_id = tr.order_id
GROUP BY cs.customer_state	
ORDER BY "% Revenue" DESC
LIMIT 5; 

-- Sao Paolo raked in more revenue at $5.2M contributing 38.28% of total revenue
-- This query went further to put the number of customers in each state into perspective to allow for better understanding  
		  ...
		  -- INSIGHT
		  -- Sao Paolo raked in more revenue at approximately $5.2M contributing 38.28% of total revenue
...
-- Step 1: Get Customer City from Customer Table 
WITH customer_city AS (
	SELECT customer_id,
		   customer_city
	FROM olist_customers_dataset
),
-- Step 2: Create a Bridge Table: Since there is no link between customers table and order items table, this link is important
orders_bridge_table AS (
	SELECT order_id,
		   customer_id
	FROM olist_orders_dataset
),
...
-- Step 4: Query the CTEs and retrieve the required columns.
SELECT cc.customer_city AS "Customer City", 
	   TO_CHAR(SUM(tr."Total Revenue"), 'FM999,999,999.00') AS "Total Revenue",
	   ROUND(100 * SUM(tr."Total Revenue") / SUM(SUM(tr."Total Revenue")) OVER(), 2) AS "% Revenue"
FROM customer_city AS cc
JOIN orders_bridge_table AS obt
	ON cc.customer_id = obt.customer_id
JOIN total_revenue AS tr
	ON obt.order_id = tr.order_id
GROUP BY cc.customer_city
ORDER BY "% Revenue" DESC
LIMIT 5; -- 14%(1.9M) of total revenue came from Sao Polo followed by Rio De Janeiro at 7.3%(993M)
...
-- 3. Freight value is worth $2.252M.
-- 4. Sao Paolo (State) raked in more revenue at approximately $5.2M contributing 38.28% of total revenue
-- 5. At least for the top 5 states by revenue performance, states with higher number of customers did better
-- 6. 42% of total customers are from Sao Paolo(State), that is what makes Sao Paolo by far the top state by Revenue generated.
-- 7. 14%(1.9M) of total revenue came from Sao Polo(City) followed by Rio De Janeiro at 7.3%($992K)
---------------------------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT product_id AS "Product",
	   TO_CHAR(SUM(price), '$FM999,999,999') AS "Revenue",
	   TO_CHAR(COUNT(order_item_id), 'FM999,999,999') AS "Units Sold",
	   ROUND(100 * SUM(price)/SUM(SUM(price)) OVER(), 2) AS "Revenue %",
	   ROUND(100 * COUNT(order_item_id)/SUM(COUNT(order_item_id)) OVER(), 2) AS "Units Sold %"
FROM olist_order_items_dataset
GROUP BY product_id
ORDER BY SUM(price) DESC;
-- This shows Total revenue per product and Total units sold
-- This query reveals there are different types of products with prices higher than each other.
-- The top product is "bb50f2e236e5eea0100680137654686c" with only 195 units totaling $63k(0.47% of total Revenue) while the  
-- top performing product by units sold 527 units yet made $37k
...
-- STEP 1: Get English product category names
WITH english_category_name AS (
    SELECT
        REPLACE(product_category_name_english, '_', ' ') AS "Product Category Name_English",
        product_category_name
    FROM product_category_name_translation
),
-- STEP 2: Combine product and order item data at the individual unit level
category_item_data AS (
    SELECT
        op.product_category_name,  -- From products dataset
        ooi.price,                 -- From order items dataset (for revenue)
        ooi.order_item_id          -- From order items dataset (for units sold)
    FROM olist_products_dataset AS op
    JOIN olist_order_items_dataset AS ooi
        ON op.product_id = ooi.product_id
)
-- STEP 3: Aggregate by English product category
SELECT ecn."Product Category Name_English" AS "Product Category",
       TO_CHAR(SUM(cid.price), '$FM999,999,999') AS "Sales Revenue",
       TO_CHAR(COUNT(cid.order_item_id), 'FM999,999,999') AS "Units Sold",
       ROUND(100 * SUM(cid.price) / SUM(SUM(cid.price)) OVER(), 2) AS "Revenue %",
	   ROUND(100 * COUNT(cid.order_item_id) / SUM(COUNT(cid.order_item_id)) OVER(), 2) AS "% Units Sold"
FROM english_category_name AS ecn
JOIN category_item_data AS cid
    ON ecn.product_category_name = cid.product_category_name
GROUP BY ecn."Product Category Name_English"
ORDER BY SUM(cid.price) DESC;
-- Health & Beauty made more sales but isn't the top category with most units sold
-- Bed, Bath & Table sold most unit but came third in Sales Revenue metric.


-- Product Sales Scatter Plot (Quantity vs Total Revenue)

SELECT REPLACE(product_category_name, '_', ' ') AS "Product Category",
	   COUNT(order_item_id) AS "Quantity", 
	   SUM(price) AS "Sales Revenue",
	   price
FROM olist_order_items_dataset AS oi
JOIN olist_products_dataset AS p			
	ON oi.product_id = p.product_id
GROUP BY "Product Category", price
ORDER BY "Sales Revenue" DESC;   
-- This query prepares data for a scatter plot showing:
--   X-axis: Quantity sold per product+price combination
...
-- Top Customers By Amount Spend
SELECT customer_id AS "Customer",
	   TO_CHAR(SUM(price), '$FM999,999,999.00') AS "Amount Spent",
	   TO_CHAR(COUNT(order_item_id), 'FM999,999,999') AS "Units Purchased"
FROM olist_orders_dataset AS oo
JOIN olist_order_items_dataset AS ooi
	ON oo.order_id = ooi.order_id
GROUP BY customer_id
ORDER BY SUM(price) DESC;
-- Customer with Id "1617b1357756262bfa56ab541c47bc16" spent most totaling $13k on 8 units which is by far not close to
-- top units bought
-- customer with ID "fc3d1daec319d62d49bfb5e1f83123e9" purchased most units with total amount spent of $32 which is unimaginably
-- below the revenue generated by the top customer by amount spent.


-- Customer Segmentation - Customer Spending Tier

-- Customer Spending Tier by Revenue & Customer Distribution
-- Step 1: Use CASE WHEN to make the segmentation by price of good purchased.
WITH customer_spending AS (
	SELECT customer_id,
		   SUM(ooi.price) AS amount_spent
	FROM olist_orders_dataset AS oo
	JOIN olist_order_items_dataset AS ooi
		ON oo.order_id = ooi.order_id
	GROUP BY customer_id
...
SELECT CASE
			WHEN amount_spent <101 THEN 'Low Spenders'
			WHEN amount_spent BETWEEN 101 AND 600 THEN 'Medium Spenders'
			ELSE 'High Spenders'
		END AS "Spending Tier",
		TO_CHAR(SUM(amount_spent), '$FM999,999,999.00') AS "Amount Spent",
		TO_CHAR(COUNT(DISTINCT customer_id), 'FM999,999,999') AS "Customers"
FROM customer_spending
-- Step 2: Group the result by the SPENDING TIER CREATED
GROUP BY "Spending Tier"			
ORDER BY "Amount Spent" DESC;
-- Medium Spenders dominate at $7.7M in revenue but has 38,197 customers which is lower than  lower tier
-- customer count at 57,889 but considerably lower revenue at $3.09M
...
-- 3. Most customers are low tier but their number does not match revenue which may mean the company is wasting resources
-- servicing this tier of customers
-- Medium customers are top customer
---------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT customer_city, 
	   TO_CHAR(COUNT(DISTINCT customer_id), '999,999,999') AS "Customers",
	   ROUND(100.0 * COUNT(DISTINCT customer_id) / SUM(COUNT(DISTINCT customer_id)) OVER(), 2) AS "% Contribution"
FROM olist_customers_dataset
...
FROM olist_orders_dataset
WHERE order_status = 'canceled'
AND(order_delivered_carrier_date IS NULL AND order_delivered_customer_date IS NULL))
...
-- AL & RR are the top states with negative reviews with negative review getting as high as approximately 24% of total review for the same states
...
-- TOP 5 SELLERS
SELECT  s.Seller_id AS "Seller's ID", 
	    s.seller_state AS "Seller's State", 
		TO_CHAR(SUM(price), '$FM999,999,999') AS "Amount Sold",
		TO_CHAR(COUNT(oi.order_item_id), 'FM999,999,999') AS "Units Sold",
		ROUND(100.0 * SUM(oi.price) / SUM(SUM(oi.price)) OVER(), 2) AS "Revenue %",
		ROUND(100.0 * COUNT(oi.order_item_id) / SUM(COUNT(oi.order_item_id)) OVER(), 2)"Units Sold %"
FROM olist_order_items_dataset AS oi
...
LIMIT 5; 
		 ...
		 -- INSIGHTS
		 -- "4869f7a5dfa277a7dca6462dcf3b52b2" is the top seller accounting for 1.68%($229k) of total revenue even as they are not the top
...
SELECT  s.seller_state AS "Seller's State", 
		TO_CHAR(SUM(price), 'FM999,999,999') AS "Sales Revenue",
		TO_CHAR(COUNT(oi.order_item_id), '$FM999,999,999') AS "Units Sold",
		ROUND(100.0 * SUM(oi.price) / SUM(SUM(oi.price)) OVER(), 2) AS "Revenue %",
		ROUND(100.0 * COUNT(oi.order_item_id) / SUM(COUNT(oi.order_item_id)) OVER(), 2)"Units Sold %"
FROM olist_order_items_dataset AS oi
...
-- SP also top REVENUE metrics as expected by 64.72%($8.7M) of total revenue
